...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $_SESSION['user_company'] = $company;
        ...
        // Update database
        if ($photo_path) {
            $sql = "UPDATE users SET name = ?, surname = ?, email = ?, photo = ?, about = ?, education = ?, skills = ?, employment_status = ?, job = ?, company = ? WHERE email = ?";
            $stmt = $conn->prepare($sql);
            if ($stmt === false) {
                die('Prepare failed: ' . $conn->error);
            }
            $stmt->bind_param('sssssssssss', $first_name, $last_name, $email, $photo_path, $about, $education, $skills, $employment_status, $job, $company, $_SESSION['user_email']);
        } else {
            // Update without changing the photo
            $sql = "UPDATE users SET name = ?, surname = ?, email = ?, about = ?, education = ?, skills = ?, employment_status = ?, job = ?, company = ? WHERE email = ?";
            $stmt = $conn->prepare($sql);
            if ($stmt === false) {
                die('Prepare failed: ' . $conn->error);
            }
            $stmt->bind_param('ssssssssss', $first_name, $last_name, $email, $about, $education, $skills, $employment_status, $job, $company, $_SESSION['user_email']);
        }
